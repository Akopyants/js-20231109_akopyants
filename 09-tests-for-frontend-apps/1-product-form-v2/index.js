...
export default class ProductForm {
  ...
  createImageListTemplate() {
    return `<ul data-element="sortableList" class="sortable-list"></ul>`;
  }
  ...
  cteateImageListItemTemplate(images = this.productInfo?.images) {
    ...
    const sortableList = new SortableList({
      items: images.map(item => {
        ...
        element.innerHTML = `<input type="hidden" name="url" value="${escapeHtml(item.url)}">
    ...
    this.subElements.sortableList.append(sortableList.element);
  }
